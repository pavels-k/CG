...
//    vec4 diffMatColor = texture2D(u_texture, v_texcoord);
    vec4 diffMatColor = vec4(0.7, 0.2, 0.0, 1.0);
...
//    resultColor += diffColor / len + ambientColor + specularColor;
    resultColor += diffMatColor * u_lightPower / len;
//    resultColor = vec4(v_position.x, v_position.y, 1.0, 1.0) * u_lightPower / pow(len, 2.0);
//    resultColor = vec4(v_position.x, v_position.y, 0.5, 1.0);
